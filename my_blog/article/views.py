...
def article_detail(request):
    ...
    count = len(Article.objects.all().annotate())
    print(count)
    # <a href = "{% url 'article:detail' %}?id={{ article.id |add:'-1' }}" > {{msg1}} < / a >
    if num == 1:
        msg1 = '没有上一篇'
    else:
        msg1 = Article.objects.get(pk=str(num - 1)).title
    if num == count:
        msg2 = '没有下一篇'
    else:
        msg2 = Article.objects.get(pk=str(num + 1)).title
    article.click_num += 1
    article.save()
    # 查询相关文章
    ...
    return render(request, 'article/info.html',
                  context={'article': article, 'list_about': list_about, 'comments': comments, 'msg1': msg1,
                           'msg2': msg2})
